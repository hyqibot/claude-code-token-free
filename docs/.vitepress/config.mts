...
const policyPath = join(
  ...
  '../../scripts/packaging/docs-publish-policy.json',
)
const docsPublishPolicy = JSON.parse(readFileSync(policyPath, 'utf8')) as {
  developerReferenceDocs: string[]
}
const developerReferenceSrcExclude = docsPublishPolicy.developerReferenceDocs.map((p) =>
  p.replace(/^docs\//, ''),
)
...
export default withMermaid(defineConfig({
  ...
  themeConfig: {
    ...
    socialLinks: [
      { icon: 'github', link: 'https://hyqibot.github.io/claude-code-token-free/' },
    ],
    footer: {
      message: 'Released under the MIT License.',
      copyright: 'Copyright 2026 Claude Code Token Free Contributors',
    },
  },
}))
