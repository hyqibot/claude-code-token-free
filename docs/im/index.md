...
桌面 UI 聊天页的模型选择是**按 session** 生效的；IM adapter 新建 session 时会走另一套默认规则：

1. **优先**：桌面「新会话 / 空会话页」里选的模型（UI 草稿），会自动同步到 `~/.claude/adapters.json` 的 `imRuntimeDefault`
2. **否则**：使用设置里**全局激活的 Provider** 及其托管 model（与桌面 UI 未单独发 `set_runtime_config` 时一致）

因此：若你在聊天页里选了 Kimi，但 IM 仍走 Zero-Token / doubao-web，请在**空会话页**重新选一次 Kimi（或把 Kimi 设为全局默认 Provider），然后在 IM 里发送 `/new` 重建会话。

查询当前 IM 默认：`GET /api/adapters/im-runtime-default`
...
- `serverUrl`
- `defaultProjectDir`
- `imRuntimeDefault`（IM 新建 session 时使用的 provider/model，由桌面 UI 草稿同步）
- `pairing`
- `telegram`
- `feishu`
- `wechat`
