...
- 流式回复约每 1.2 秒或满 200 字批量发送一次（模型生成完成后会立即刷出剩余内容）。
- 文本回复按 3500 字左右分片发送。
- thinking 和工具执行期间会尽量发送微信 typing 状态。
- 图片会以内联图片形式进入模型输入；非图片文件会以本地临时文件路径传给会话。
- Agent 生成的本地 PNG/JPG 等（markdown 图片或正文里的 `D:/.../xxx.png` 路径）会自动上传并以独立图片消息发出。
...
桌面聊天页按 session 选模型；微信 IM 新建 session 时优先用空会话页草稿（`imRuntimeDefault`），否则用全局激活 Provider。改模型后请发 `/new`，详见 [IM 总览 — 模型说明](../im/index.md#5-im-使用的模型--provider)。
