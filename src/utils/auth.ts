...
import { isLoopbackAnthropicBaseUrlEnv } from './proxy.js'
import {
...
/**
 * Skip background OAuth refresh when the CLI is not using Anthropic 1P OAuth for
 * inference (Zero-Token / host-managed presets, loopback gateways, API keys).
 * Refresh still hits https://api.anthropic.com and can throw Bun's
 * "unknown certificate verification error" under MITM proxies even when chat
 * goes to http://127.0.0.1:3002.
 */
export function shouldSkipOAuthTokenRefresh(
  env: NodeJS.ProcessEnv = process.env,
): boolean {
  if (isEnvTruthy(env.CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return true
  }
  if (isLoopbackAnthropicBaseUrlEnv(env)) {
    return true
  }
