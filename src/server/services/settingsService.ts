...
export class SettingsService {
  ...
  /**
   * 同步读取 ~/.claude/settings.json 顶层 `insecureTls`。
   * 启动早期（startServer）和 spawn CLI 子进程时都需要在阻塞路径上拿到这个值，
   * 因此用 sync fs；任何 IO/解析异常一律返回 false（默认安全）。
   */

  isInsecureTlsEnabled(): boolean {
    try {
      // Check user-level settings first
      const rawUser = fsSync.readFileSync(this.getUserSettingsPath(), 'utf-8')
      const parsedUser = JSON.parse(rawUser) as { insecureTls?: unknown }
      if (parsedUser.insecureTls === true) return true

      // If project root is defined, also check project-level settings
      if (this.projectRoot) {
        const rawProj = fsSync.readFileSync(this.getProjectSettingsPath(), 'utf-8')
        const parsedProj = JSON.parse(rawProj) as { insecureTls?: unknown }
        return parsedProj.insecureTls === true
      }
      return false
    } catch {
      return false
