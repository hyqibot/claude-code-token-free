...
import { recordCliSpawnConnectivityDiag } from '../utils/chatConnectivityDiag.js'
...
type SessionProcess = {
  ...
  /** spawn 时子进程实际使用的 Anthropic 兼容端点/模型（日志与诊断用，不含密钥）。 */
  spawnAnthropicBaseUrl?: string
  spawnAnthropicModel?: string
  stdoutLines: string[]
  stderrLines: string[]
...
export class ConversationService {
  ...
  async startSession(
    ...
    recordCliSpawnConnectivityDiag(sessionId, {
      optionsProviderId: options?.providerId,
      optionsModel: options?.model,
      anthropicBaseUrl: childEnv.ANTHROPIC_BASE_URL,
      anthropicModel: childEnv.ANTHROPIC_MODEL,
      providerManagedByHost: childEnv.CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST === '1',
      officialEntrypoint: Boolean(childEnv.CLAUDE_CODE_ENTRYPOINT),
      strippedInheritedProviderEnv: this.shouldStripInheritedProviderEnv(options?.providerId),
      noProxyDefined: Boolean((childEnv.NO_PROXY ?? childEnv.no_proxy)?.trim()),
      serverPort: ProviderService.getServerPort(),
    })

    console.log(
      `[ConversationService] CLI spawn inference: ANTHROPIC_BASE_URL=${childEnv.ANTHROPIC_BASE_URL ?? '(unset)'} ANTHROPIC_MODEL=${childEnv.ANTHROPIC_MODEL ?? '(unset)'} managed=${childEnv.CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST === '1'}`,
    )
    ...
    const session: SessionProcess = {
      ...
      spawnAnthropicBaseUrl:
        typeof childEnv.ANTHROPIC_BASE_URL === 'string' ? childEnv.ANTHROPIC_BASE_URL : undefined,
      spawnAnthropicModel:
        typeof childEnv.ANTHROPIC_MODEL === 'string' ? childEnv.ANTHROPIC_MODEL : undefined,
      stdoutLines: [],
      stderrLines: [],
  ...
  /** spawn 时实际写入子进程的 ANTHROPIC_BASE_URL，供聊天报错时做根因提示（如 Zero-Token 网关未监听）。 */
  getSpawnAnthropicBaseUrl(sessionId: string): string | undefined {
    return this.sessions.get(sessionId)?.spawnAnthropicBaseUrl
  }

  /**
   * 与启动失败时的 capturedOutput 同源：已捕获的 CLI stderr/stdout，脱敏后截断，供聊天 API 报错时展示。
   */
  getCliProcessOutputTailForChatError(sessionId: string): string {
    const session = this.sessions.get(sessionId)
    if (!session) return ''
    const raw = this.buildCapturedProcessOutputDetail(session).trim()
    if (!raw) return ''
    const redacted = this.redactProcessOutput(raw)
    const max = 3200
  ...
  private async buildChildEnv(
    ...
    /** 任意会话都可能走 `http://127.0.0.1:<SERVER_PORT>/proxy`；合并 loopback 到 NO_PROXY，避免 Windows 下 HTTP_PROXY 劫持本机 cc-haha 代理。 */
    mergeNoProxyEnvVars(cleanEnv)
    ...
    const explicitProviderEnv =
      typeof options?.providerId === 'string'
        ...
        : null
    if (explicitProviderEnv) {
      if (options?.model?.trim()) {
        explicitProviderEnv.ANTHROPIC_MODEL = options.model.trim()
      }
      // 托管 provider（如 Zero-Token）须清掉父进程 / settings 残留的 Bedrock/Azure 等路由，
      // 否则 CLI 仍会走 azureOpenAI（例如占位 AZURE_OPENAI_BASE_URL=your-resource...）。
    ...
    stripForwardProxyForLoopbackAnthropicBaseUrl(childEnv)
    mergeNoProxyEnvVars(childEnv)

    /**
     * 显式开关：设置页 General 里的 "在 HTTPS 代理下信任所有证书"（~/.claude/settings.json
     * 顶层 insecureTls=true）。MITM 代理（自签 CA）下 Bun fetch 会因证书不可信抛
     * "unknown certificate verification error"，开启后给 CLI 子进程注入
     * NODE_TLS_REJECT_UNAUTHORIZED=0 (Node) 和 COPAW_INSECURE_TLS=1 (Bun/cc-haha) 绕开校验。
     * 绝不做"自动检测代理"的隐式兜底。
     */
    if (this.settingsService.isInsecureTlsEnabled()) {
      childEnv.NODE_TLS_REJECT_UNAUTHORIZED = '0'
      childEnv.COPAW_INSECURE_TLS = '1'
      for (const k of ['HTTP_PROXY', 'HTTPS_PROXY', 'http_proxy', 'https_proxy', 'ALL_PROXY', 'all_proxy'] as const) {
        delete childEnv[k]
      }
    }

    return childEnv as Record<string, string>
  }
