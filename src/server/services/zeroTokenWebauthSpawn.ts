...
export async function resolveWebauthNodeRunnerBundlePath(): Promise<string | null> {
  ...
  try {
    ...
    // dev checkout without vendor
  }
  return null
}

/** webauth 依赖检查：桌面旁路（exe/runtime）优先，避免误走 vendor npm install。 */
export function resolveWebauthDepsSpawnMode(): ZeroTokenWebauthSpawnMode {
  if (resolveWebauthSpawnMode() === 'node') return 'node'
  if (playwrightMarkerExistsForWebauth()) return 'exe'
  if (process.env.CLAUDE_APP_ROOT?.trim()) return 'exe'
  return 'node'
}

export function findWebauthRunnerExeSync(): string | null {
  for (const candidate of webauthRunnerExeCandidates()) {
...
/** 安装包内置 Node（zero-token-runtime/node/）。 */
export function resolveBundledWebauthNodeBinary(appRoot?: string): string | null {
  const root = appRoot?.trim() || resolvePackagedAppRoot(dirname(process.execPath))
  const binName = process.platform === 'win32' ? 'node.exe' : 'node'
  for (const rel of [
    join('zero-token-runtime', 'node', binName),
    join('resources', 'zero-token-runtime', 'node', binName),
  ]) {
...
export function resolveWebauthNodeBinaryForSpawn(appRoot?: string): string {
  return (
    process.env.CC_HAHA_WEBAUTH_NODE?.trim() ||
    resolveBundledWebauthNodeBinary(appRoot) ||
...
export async function resolveWebauthSpawnPlan(params: {
  ...
  const nodeBin =
    params.nodeBin?.trim() ||
    resolveWebauthNodeBinaryForSpawn(appRoot)
