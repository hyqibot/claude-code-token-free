...
export function resolvePackagedAppRoot(fallbackExecDir?: string): string {
  const explicit =
    process.env.CC_HAHA_ROOT?.trim() || process.env.CLAUDE_APP_ROOT?.trim()
  if (explicit) return explicit

  try {
    const repoRoot = getCcHahaRepoRoot()
    const devArtifactsRoot = join(repoRoot, 'desktop', 'build-artifacts')
    if (
      existsSync(join(devArtifactsRoot, 'zero-token-runtime', 'gateway.bundle.mjs'))
    ) {
      return devArtifactsRoot
    }
    return repoRoot
  } catch {
    return fallbackExecDir?.trim() || dirname(process.execPath)
  }
}
