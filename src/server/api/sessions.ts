...
/** 防止会话工作区 git 阻塞（网络盘 / 锁文件）导致 GET /git-info 长时间挂起 */
const GIT_INFO_GIT_TIMEOUT_MS = 6_000

async function gitStdoutForGitInfo(gitArgs: string[], cwd: string): Promise<string | null> {
...
async function getGitInfo(sessionId: string): Promise<Response> {
  ...
  try {
    const branchText = await gitStdoutForGitInfo(['rev-parse', '--abbrev-ref', 'HEAD'], workDir)
    const branch = sessionBranch || branchText?.trim() || ''

    let repoName = ''
    const remoteText = await gitStdoutForGitInfo(['remote', 'get-url', 'origin'], workDir)
    if (remoteText) {
    ...
    const statusText = await gitStdoutForGitInfo(['status', '--porcelain'], workDir)
