...
import { resolveImRuntimeDefault, setImRuntimeDefault } from '../services/imRuntimeService.js'
...
const ALLOWED_TOP_KEYS = new Set(['serverUrl', 'defaultProjectDir', 'imRuntimeDefault', 'telegram', 'feishu', 'wechat', 'dingtalk', 'pairing'])
...
export async function handleAdaptersApi(
  ...
  try {
    ...
    if (tail[0] === 'im-runtime-default') {
      if (req.method === 'GET') {
        return Response.json(await resolveImRuntimeDefault())
      }
      if (req.method === 'PUT') {
        ...
        if (body?.clear) {
          await setImRuntimeDefault(null)
        } else {
          const modelId = typeof body?.modelId === 'string' ? body.modelId.trim() : ''
          if (!modelId) throw ApiError.badRequest('Missing or invalid modelId')
          const providerId = body?.providerId === null
            ? null
            : typeof body?.providerId === 'string'
              ? body.providerId
              : null
          await setImRuntimeDefault({ providerId, modelId })
        }
        return Response.json(await resolveImRuntimeDefault())
      }
  ...
  if (req.method === 'POST' && tail[0] === 'login' && tail[1] === 'poll') {
    ...
    if (result.connected) {
      const pairedUsers = result.userId
        ? [{ userId: result.userId, displayName: 'WeChat Owner', pairedAt: Date.now() }]
        : []
      await adapterService.updateConfig({
        wechat: {
