...
import type { ApiFormat, ModelMapping, ProviderAuthStrategy, SavedProvider } from '../types/provider.js'

const providerService = new ProviderService()

function redactProvider(provider: SavedProvider): SavedProvider {
  return {
    ...provider,
    // Do not return stored secrets to the desktop UI. Empty keeps edit forms in
    // "leave blank to keep current" mode instead of accidentally resaving a mask.
    apiKey: '',
  }
}

function redactProviders(providers: SavedProvider[]): SavedProvider[] {
  ...
  try {
    ...
    if (!id) {
      if (req.method === 'GET') {
        ...
        return Response.json({ providers: redactProviders(providers), activeId })
      }
      if (req.method === 'POST') {
    ...
    if (req.method === 'GET') {
      ...
      return Response.json({ provider: redactProvider(provider) })
    }
    if (req.method === 'PUT') {
...
async function handleCreate(req: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ provider: redactProvider(provider) }, { status: 201 })
  } catch (err) {
