...
import { getZeroTokenGatewayHintForChatError } from '../utils/zeroTokenGatewayHint.js'
...
import { sessionOverrideStore } from '../services/sessionOverrideStore.js'
import { sharedZeroTokenService } from '../services/zeroTokenService.js'

const settingsService = new SettingsService()
const providerService = new ProviderService()

/**
 * 把持久化的 session runtime override 灌进内存 map，确保 server 重启 / 客户端重连后
 * 即使没收到 set_runtime_config 也能拿到上一次选择的 provider/model。
 * 第一次调用 getRuntimeSettings 时 await 这个 promise；一次成功后不再触发 IO。
 */
let runtimeOverridesLoaded: Promise<void> | null = null
function ensureRuntimeOverridesLoaded(): Promise<void> {
  if (!runtimeOverridesLoaded) {
    runtimeOverridesLoaded = (async () => {
      try {
        const all = await sessionOverrideStore.loadAll()
        for (const [sid, ov] of Object.entries(all)) {
          if (!runtimeOverrides.has(sid)) {
            runtimeOverrides.set(sid, ov)
          }
        }
      } catch (err) {
        console.warn(
          `[WS] failed to load persisted session overrides: ${
...
/** 每个 session 仅做一次 prewarm 缓冲补发，避免客户端重连重复刷 init。 */
const prewarmSdkReplayDone = new Set<string>()
const DEFAULT_PREWARM_IDLE_TIMEOUT_MS = 5 * 60_000

/** 诊断模式才在聊天正文后附带 CLI 输出；默认不把 stderr 诊断堆进用户可见错误。 */
function appendCliCapturedOutputForChatError(text: string, sessionId: string): string {
  if (!isChatConnectivityDiagEnabled()) return text
  const tail = conversationService.getCliProcessOutputTailForChatError(sessionId)
  if (!tail) return text
  return `${text}\n\n【CLI stderr/stdout】\n${tail}`
}

/**
 * 当 chat 错误的 CLI 子进程指向本机 Zero-Token 网关，且端口未监听时，给错误消息追加一行根因提示。
 * 不影响其它情形（普通 provider / 网关在跑都直接返回原消息）。
 */
async function maybeAppendZeroTokenGatewayHintForError(
  text: string,
  sessionId: string,
): Promise<string> {
  const baseUrl = conversationService.getSpawnAnthropicBaseUrl(sessionId)
  const hint = await getZeroTokenGatewayHintForChatError(baseUrl, text)
  return hint ? `${text}\n\n${hint}` : text
}
...
/** Serialize inbound client messages per session (runtime config before user turn). */
const sessionInboundChains = new Map<string, Promise<void>>()

function enqueueSessionInbound(sessionId: string, task: () => Promise<void>): void {
  const prev = sessionInboundChains.get(sessionId) ?? Promise.resolve()
  const next = prev
    .catch(() => undefined)
    .then(task)
    .catch((err) => {
      console.error(`[WS] Unhandled error in session inbound queue for ${sessionId}:`, err)
    })
  sessionInboundChains.set(sessionId, next)
}
...
export const handleWebSocket = {
  open(ws: ServerWebSocket<WebSocketData>) {
    ...
    bindClientOutputForActiveSession(sessionId)
  ...
  message(ws: ServerWebSocket<WebSocketData>, rawMessage: string | Buffer) {
    ...
    try {
      ...
      switch (message.type) {
        case 'user_message':
          enqueueSessionInbound(ws.data.sessionId, async () => {
            try {
        ...
        case 'set_runtime_config':
          enqueueSessionInbound(ws.data.sessionId, async () => {
            await handleSetRuntimeConfig(ws, message)
          })
          break

        case 'prewarm_session':
          enqueueSessionInbound(ws.data.sessionId, async () => {
            await handlePrewarmSession(ws)
          })
          break
...
async function handleSetRuntimeConfig(
  ...
  sessionOverrideStore.set(sessionId, nextOverride)
  recordChatConnectivityDiag({
    phase: 'ws_set_runtime_config',
    sessionId,
    summary: 'client set_runtime_config',
    details: {
      providerId: nextOverride.providerId,
...
async function restartSessionWithPermissionMode(
  ...
  try {
    ...
    prewarmSdkReplayDone.delete(sessionId)
    const clientWs = activeSessions.get(sessionId)
    if (clientWs) {
      rebindSessionOutput(sessionId, clientWs)
    }
...
/** Prewarm 完成后再连上的客户端：补发已缓冲的 init / 错误，避免首条 user_message 前 UI 收不到 SDK 侧失败。 */
function replayPrewarmSdkNotificationsToClient(
  sessionId: string,
  ws: ServerWebSocket<WebSocketData>,
): void {
  for (const msg of conversationService.getRecentSdkMessages(sessionId)) {
    cacheSessionInitMetadata(sessionId, msg)
    const serverMsgs = translateCliMessage(msg, sessionId)
    for (const sm of serverMsgs) {
      if (sm.type === 'system_notification' || sm.type === 'error') {
        void sendMessageWithGatewayHint(ws, sessionId, sm)
      }
    }
  }
}

/** 仅对 type=error 异步追加 Zero-Token 网关未监听提示；其它消息原样发送。 */
async function sendMessageWithGatewayHint(
  ws: ServerWebSocket<WebSocketData>,
  sessionId: string,
  msg: ServerMessage,
): Promise<void> {
  if (msg.type === 'error' && typeof msg.message === 'string') {
    const enhanced = await maybeAppendZeroTokenGatewayHintForError(msg.message, sessionId)
    sendMessage(ws, { ...msg, message: enhanced })
    return
  }
  sendMessage(ws, msg)
}
...
/**
 * 客户端已连接且 CLI 已存在时挂上输出转发。
 * 覆盖：① prewarm 完成后客户端早已连上（open 时尚无子进程）；② prewarm 完成后客户端再连。
 */
function bindClientOutputForActiveSession(sessionId: string): void {
  const clientWs = activeSessions.get(sessionId)
  if (!clientWs || !conversationService.hasSession(sessionId)) return
  if (prewarmedSessions.has(sessionId) && !prewarmSdkReplayDone.has(sessionId)) {
    replayPrewarmSdkNotificationsToClient(sessionId, clientWs)
    prewarmSdkReplayDone.add(sessionId)
  }
  rebindSessionOutput(sessionId, clientWs)
}
...
/** Zero-Token 聊天前若 3002 未监听则自动拉起网关（避免 CLI 连不上或误报 TLS）。 */
async function ensureZeroTokenGatewayListening(
  runtimeSettings: RuntimeSettings,
): Promise<void> {
  const pid = runtimeSettings.providerId
  if (typeof pid !== 'string' || !pid.trim()) return

  let provider
  try {
    provider = await providerService.getProvider(pid)
  } catch {
    return
  }
  if (provider.presetId !== 'zero-token-web') return

  const { assertGatewayLicenseForGateway } = await import('../services/gatewayLicense/gatewayLicenseService.js')
  ...
  if (status.listening) {
    ...
    console.warn(`[WS] Zero-Token gateway TCP up but /health not ready on ${host}:${port}; starting anyway`)
    return
  }

  console.log(
    '[WS] Zero-Token gateway not listening on 3002; auto-starting before CLI session',
  )
  await sharedZeroTokenService.start()
}
...
async function ensureCliSessionStarted(
  ...
  const startup = (async () => {
    ...
    const runtimeSettings = await getRuntimeSettings(sessionId)
    await ensureZeroTokenGatewayListening(runtimeSettings)
    const sdkUrl =
      `ws://${ws.data.serverHost}:${ws.data.serverPort}/sdk/${sessionId}` +
      `?token=${encodeURIComponent(crypto.randomUUID())}`
  ...
  switch (cliMsg.type) {
    case 'assistant': {
      if (cliMsg.error || cliMsg.isApiErrorMessage) {
        ...
        return [{
          type: 'error',
          message: appendCliCapturedOutputForChatError(message, sessionId),
          code,
        }]
      }
    ...
    case 'result': {
      ...
      if (cliMsg.is_error) {
        ...
        return [
          {
            type: 'error',
            message: appendCliCapturedOutputForChatError(resultMessage, sessionId),
            code: 'CLI_ERROR',
          },
...
function rebindSessionOutput(
  ...
  conversationService.onOutput(sessionId, (cliMsg) => {
    ...
    for (const msg of serverMsgs) {
      void sendMessageWithGatewayHint(ws, sessionId, msg)
    }
...
/**
 * CoPaw 网关 `/v1/messages` 仅接受 10 个 canonical web id（WEB_ONLY）。
 * 若当前激活为 Zero-Token 但托管 model 仍为官方 id（切换服务商后未刷新），CLI 会打到网关并报「暂不支持」——此处对齐为服务商主槽或 deepseek-chat。
 */
async function coerceZeroTokenWebRuntimeModel(settings: RuntimeSettings): Promise<RuntimeSettings> {
  const pid = settings.providerId
  if (typeof pid !== 'string' || !pid.trim()) return settings
  ...
  if (provider.presetId !== 'zero-token-web') return settings

  const fallback = (provider.models.main || 'deepseek-chat').trim() || 'deepseek-chat'

  const raw = settings.model?.trim()
  if (!raw) {
    return settings.model === fallback ? settings : { ...settings, model: fallback }
  }

  const colonIdx = raw.indexOf(':')
  const baseId = colonIdx >= 0 ? raw.slice(0, colonIdx).trim() : raw

  if (isGatewayCanonicalWebModelId(baseId)) return settings

  console.warn(
    `[WS] Zero-Token active but model base '${baseId}' is not a gateway WEB_ONLY id; ` +
      `coercing CLI model to '${fallback}' (align CoPaw /v1/messages)`,
  )
  ...
  if (runtimeOverride) {
    if (typeof runtimeOverride.providerId === 'string') {
      ...
      if (!providerExists) {
        ...
        runtimeOverrides.delete(sessionId!)
        sessionOverrideStore.delete(sessionId!)
        return await coerceZeroTokenWebRuntimeModel(await getDefaultRuntimeSettings())
      }
    }
    ...
    return await coerceZeroTokenWebRuntimeModel({
...
export function __resetWebSocketHandlerStateForTests(): void {
  ...
  prewarmSdkReplayDone.clear()
  sessionInboundChains.clear()
}
