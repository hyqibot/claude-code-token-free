...
import { mergeProcessEnvNoProxyLoopback } from './utils/loopbackFetchEnv.js'
import { SettingsService } from './services/settingsService.js'
import { resumeGatewayLicenseIfConfigured } from './services/gatewayLicense/gatewayLicenseService.js'
...
export function startServer(port = PORT, host = HOST) {
  ...
  /**
   * 桌面端 Settings → General 里的 "在 HTTPS 代理下信任所有证书" 开关
   * (~/.claude/settings.json 顶层 insecureTls=true) 让 server 自身的 fetch
   * (provider 测试 / 标题生成等) 也跳过 TLS 校验；CLI 子进程的开关在
   * ConversationService.buildChildEnv 里独立读取一次。
   */
  if (
    new SettingsService().isInsecureTlsEnabled() &&
    process.env.NODE_TLS_REJECT_UNAUTHORIZED !== '0'
  ) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'
    console.warn('[Server] insecureTls=true → NODE_TLS_REJECT_UNAUTHORIZED=0 (TLS verification disabled)')
  }
  ...
  void resumeGatewayLicenseIfConfigured()
