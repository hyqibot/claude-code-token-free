...
describe('WebSocket Chat Integration', () => {
  ...
  /** 不等整轮 mock 流结束：满足谓词即关连接，避免 Bun 默认 5s 单测超时（Windows 上 git+全轮较慢）。 */
  async function runTurnUntilPredicate(
    sessionId: string,
    content: string,
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ws.close()
        reject(new Error(`Timed out waiting for predicate for session ${sessionId}`))
      }, 30000)
  ...
  it('should send user_message and receive streamed SDK response', async () => {
    const messages = await runTurnUntilPredicate(
      `chat-stream-${crypto.randomUUID()}`,
      'Hello from test',
      (entries) =>
        entries.some((entry) => entry.type === 'message_complete') &&
        entries.some((entry) => entry.type === 'thinking'),
    )
  ...
  describe('repository session startup status verbs', () => {
    let repoDir: string

    beforeAll(async () => {
      repoDir = await createCleanGitRepo()
    })
    ...
    it('emits a worktree startup status before launching a repository session', async () => {
      ...
      const messages = await runTurnUntilPredicate(
        ...
        (m) => m.some((msg) => msg.type === 'status' && msg.verb === 'Creating worktree'),
      )
    ...
    it('keeps the default startup status for current-worktree repository sessions', async () => {
      ...
      const messages = await runTurnUntilPredicate(sessionId, 'Hello from current worktree launch test', (m) => {
        const verbs = m.filter((msg) => msg.type === 'status').map((msg) => msg.verb)
        return verbs.includes('Thinking') && !verbs.includes('Creating worktree')
      })
      const statusVerbs = messages
        .filter((msg) => msg.type === 'status')
        .map((msg) => msg.verb)
  ...
  it('should complete the client turn when the CLI exits after startup', async () => {
    const messages = await withMockExitAfterFirstUser(500, () =>
      runTurnUntilPredicate(
        `chat-late-exit-${crypto.randomUUID()}`,
        'trigger late exit',
        (entries) =>
          entries.some(
            (m) =>
