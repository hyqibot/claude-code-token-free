...
import {
  ...
  deepSeekUseStreamSieve,
} from '../../../vendor/copaw-zero-token/python/src/copaw/zero_token_gateway/deepseek-tool-mode.mjs'

describe('deepseek-prompt', () => {
  let prevDeepseekToolMode: string | undefined

  beforeEach(() => {
    prevDeepseekToolMode = process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE = 'dsml'
  })

  afterEach(() => {
    if (prevDeepseekToolMode === undefined) delete process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    else process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE = prevDeepseekToolMode
  })
  ...
  test('xml mode: download URL first turn appends XML task directive', () => {
    const prev = process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE = 'xml'
    try {
      const tools = [
        ...
        { type: 'function', function: { name: 'Bash', description: 'run', parameters: { type: 'object', properties: { command: {} }, required: ['command'] } } },
      ]
      const url = 'https://marketing.dfcfw.com/res/download/A620260402NXEUQC.md'
      const prompt = buildDeepSeekPromptForTurn(
        [{ role: 'user', content: `下载 ${url} 文档后，告诉我里面的内容` }],
        tools,
        null,
        { dsmlFullSent: false },
      )
...
describe('deepseek-tool-mode', () => {
  test('resolveDeepSeekToolMode defaults to xml', () => {
    const prev = process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    delete process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    try {
      expect(resolveDeepSeekToolMode({})).toBe('xml')
      expect(isDeepSeekXmlMode({})).toBe(true)
      expect(deepSeekUseStreamSieve({})).toBe(false)
    } finally {
      if (prev === undefined) delete process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
      else process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE = prev
    }
  ...
  test('resolveDeepSeekToolMode honors explicit dsml', () => {
    expect(resolveDeepSeekToolMode({ COPAW_ZT_DEEPSEEK_TOOL_MODE: 'dsml' })).toBe('dsml')
    expect(deepSeekUseStreamSieve({ COPAW_ZT_DEEPSEEK_TOOL_MODE: 'dsml' })).toBe(true)
  })

  test('buildDeepSeekPromptForTurn uses xml prompt when env unset', () => {
    const prev = process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    delete process.env.COPAW_ZT_DEEPSEEK_TOOL_MODE
    try {
      const tools = [{ type: 'function', function: { name: 'Bash', description: 'run' } }]
      const prompt = buildDeepSeekPromptForTurn([{ role: 'user', content: 'hi' }], tools, null, {
        dsmlFullSent: false,
      })
      expect(prompt).toContain('## Tool Use Instructions')
      expect(prompt).not.toContain('<|DSML|tool_calls>')
