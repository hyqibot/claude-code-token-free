...
function withoutBuiltInZeroToken<T extends { presetId: string }>(providers: T[]): T[] {
  return providers.filter((p) => p.presetId !== 'zero-token-web')
}
...
describe('ProviderService', () => {
  ...
  describe('listProviders', () => {
    test('should seed built-in Zero-Token row when index is empty', async () => {
      const svc = new ProviderService()
      const result = await svc.listProviders()
      expect(result.activeId).toBeNull()
      expect(result.providers).toHaveLength(1)
      expect(result.providers[0].presetId).toBe('zero-token-web')
    })
    ...
    test('should recover from a malformed providers index after an upgrade', async () => {
      ...
      expect(result.providers.some((p) => p.presetId === 'zero-token-web')).toBe(true)
      expect(withoutBuiltInZeroToken(result.providers)).toHaveLength(0)
      expect(result.activeId).toBeNull()
    ...
    test('should normalize a legacy activeProviderId field', async () => {
      ...
      expect(result.providers.find((p) => p.id === provider.id)?.name).toBe('Legacy Provider')
      expect(result.providers.some((p) => p.presetId === 'zero-token-web')).toBe(true)
    })
    ...
    test('should return all added providers', async () => {
      ...
      const userRows = withoutBuiltInZeroToken(providers)
      expect(userRows).toHaveLength(2)
      expect(userRows[0].name).toBe('Provider A')
      expect(userRows[1].name).toBe('Provider B')
  ...
  describe('deleteProvider', () => {
    test('should delete an inactive provider', async () => {
      ...
      expect(withoutBuiltInZeroToken(providers)).toHaveLength(1)
      expect(withoutBuiltInZeroToken(providers)[0].name).toBe('First')
    })
    ...
    test('should reject deleting built-in Zero-Token provider', async () => {
      const svc = new ProviderService()
      const { providers } = await svc.listProviders()
      const zt = providers.find((p) => p.presetId === 'zero-token-web')
      expect(zt).toBeDefined()
      try {
        await svc.deleteProvider(zt!.id)
        expect(true).toBe(false)
      } catch (err: unknown) {
        const apiErr = err as { statusCode: number }
  ...
  describe('activateProvider', () => {
    ...
    test('should include preset default env on activation and runtime env', async () => {
      ...
      expect(runtimeEnv.CC_HAHA_SEND_DISABLED_THINKING).toBe('1')
  ...
  describe('getProviderForProxy', () => {
    ...
    test('should return the active provider proxy config', async () => {
      ...
      expect(active!.presetId).toBe('custom')
    })
  })
  ...
  describe('testProvider', () => {
    test('should use preset default auth for saved no-key Anthropic-compatible providers', async () => {
      ...
      try {
        ...
        for (const c of calls) {
          expect(c.headers.Authorization).toBe('Bearer lmstudio')
    ...
    test('non-Zero-Token active provider tests each saved slot (ignores managed settings.model)', async () => {
      const originalFetch = globalThis.fetch
      ...
      try {
        ...
        const provider = await svc.addProvider(sampleInput())
        await svc.activateProvider(provider.id)
        await svc.updateManagedSettings({ model: 'some-other-model-id' })

        const result = await svc.testProvider(provider.id)
        expect(result.connectivity.success).toBe(true)
        expect(result.slotResults).toHaveLength(4)
        expect((bodies[0] as { model?: string }).model).toBe('model-main')
        expect((bodies[1] as { model?: string }).model).toBe('model-haiku')
        expect((bodies[2] as { model?: string }).model).toBe('model-sonnet')
        expect((bodies[3] as { model?: string }).model).toBe('model-opus')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe('testProviderConfig', () => {
    ...
    test('should test only non-empty slots when models mapping is provided', async () => {
      const originalFetch = globalThis.fetch
      const bodies: unknown[] = []
      globalThis.fetch = mock(async (_url: string | URL | Request, init?: RequestInit) => {
        bodies.push(JSON.parse(String(init?.body ?? '{}')))
      ...
      try {
        ...
        await svc.testProviderConfig({
          baseUrl: 'https://api.example.com/anthropic',
          apiKey: 'sk-x',
          models: { main: 'a', haiku: '', sonnet: 'b', opus: 'c' },
          authStrategy: 'api_key',
          apiFormat: 'anthropic',
        })
        expect(bodies).toHaveLength(3)
        expect((bodies[0] as { model?: string }).model).toBe('a')
        expect((bodies[1] as { model?: string }).model).toBe('b')
        expect((bodies[2] as { model?: string }).model).toBe('c')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
...
describe('Providers API', () => {
  ...
  test('GET /api/providers should include built-in Zero-Token row initially', async () => {
    const { req, url, segments } = makeRequest('GET', '/api/providers')
    const res = await handleProvidersApi(req, url, segments)

    expect(res.status).toBe(200)
    const body = (await res.json()) as { providers: Array<{ presetId: string }> }
    expect(body.providers).toHaveLength(1)
    expect(body.providers[0].presetId).toBe('zero-token-web')
  })
  ...
  test('GET /api/providers should list added providers', async () => {
    ...
    const userRows = withoutBuiltInZeroToken(body.providers as Array<{ presetId: string }>)
    expect(userRows).toHaveLength(1)
    expect(userRows[0].name).toBe('Test Provider')
    expect(userRows[0].apiKey).toBe('')
  ...
  test('POST /api/providers should create a provider', async () => {
    ...
    const body = (await res.json()) as { provider: { name: string; apiKey: string; models: { main: string }; autoCompactWindow: number } }
  ...
  test('POST /api/providers should reject zero-token-web preset (built-in row)', async () => {
    const { req, url, segments } = makeRequest('POST', '/api/providers', {
      presetId: 'zero-token-web',
      name: 'Dup ZT',
      baseUrl: 'http://127.0.0.1:3002',
      apiKey: 'zero-token-local',
      apiFormat: 'anthropic',
      models: {
        main: 'deepseek-chat',
        haiku: 'deepseek-chat',
  ...
  test('GET /api/providers/:id should return a provider', async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as { provider: { id: string; name: string; apiKey: string } }
  ...
  test('PUT /api/providers/:id should update a provider', async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as { provider: { name: string; apiKey: string } }
