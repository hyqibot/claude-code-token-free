...
describe('Adapters API', () => {
  ...
  it('GET/PUT /api/adapters/im-runtime-default stores draft runtime for IM adapters', async () => {
    const put = makeRequest('PUT', '/api/adapters/im-runtime-default', {
      providerId: null,
      modelId: 'kimi-k2.6',
    })
    const putRes = await handleAdaptersApi(put.req, put.url, put.segments)
    expect(putRes.status).toBe(200)
    const saved = await putRes.json() as any
    expect(saved.source).toBe('draft')
    expect(saved.modelId).toBe('kimi-k2.6')
    expect(saved.providerId).toBeNull()

    const get = makeRequest('GET', '/api/adapters/im-runtime-default')
    ...
    expect(loaded.modelId).toBe('kimi-k2.6')

    const clear = makeRequest('PUT', '/api/adapters/im-runtime-default', { clear: true })
    const clearRes = await handleAdaptersApi(clear.req, clear.url, clear.segments)
    expect(clearRes.status).toBe(200)
    const cleared = await clearRes.json() as any
    expect(cleared.source).toBe('global')
  })
})
