...
import { recordProxyRequestInDiag, recordProxyUpstreamDiag } from '../utils/chatConnectivityDiag.js'
...
export async function handleProxyRequest(req: Request, url: URL): Promise<Response> {
  ...
  const routeMatch = Boolean(isActiveProxyPath || providerMatch)
  ...
  const diagMeta: ProxyDiagMeta = {
    pathKind: providerId ? 'scoped' : 'active',
    ...(providerId ? { providerId } : {}),
  ...
  try {
    if (config.apiFormat === 'openai_chat') {
      return await handleOpenaiChat(body, baseUrl, config.apiKey, isStream, diagMeta)
    } else {
      return await handleOpenaiResponses(body, baseUrl, config.apiKey, isStream, diagMeta)
    }
  } catch (err) {
    console.error('[Proxy] Upstream request failed:', baseUrl, err)
    const msg = err instanceof Error ? err.message : String(err)
    recordProxyUpstreamDiag({
      ...diagMeta,
      upstreamBaseUrl: baseUrl,
      upstreamPath: '(before route)',
      errorMessage: msg,
    })
    const loopbackHint =
      /127\.0\.0\.1|localhost|\[::1\]/i.test(baseUrl)
        ? ` [Zero-Token/本机网关 ${baseUrl}：请确认进程已监听；若设置了 HTTP_PROXY，勿让代理拦截 loopback——服务端已在启动时合并 NO_PROXY。]`
        : ''
    return Response.json(
      {
        type: 'error',
...
async function handleOpenaiChat(
  ...
  if (!upstream.ok) {
    ...
    recordProxyUpstreamDiag({
      ...
      errorMessage: `HTTP ${upstream.status}: ${errText.slice(0, 500)}`,
    })
    return Response.json(
      {
        type: 'error',
  ...
  if (isStream) {
    if (!upstream.body) {
      recordProxyUpstreamDiag({
        ...
        upstreamPath: '/v1/chat/completions',
        httpStatus: upstream.status,
        errorMessage: 'Upstream returned no body for stream',
      })
      return Response.json(
