...
- **聊天页 PNG/曲线图不显示？** 桌面端会把回复里的本地绝对路径（如 `D:\CCwork\xxx.png`）转为 `/api/filesystem/file` 加载。除用户主目录、session 工作区外，**会话 transcript 里出现过的输出目录**（如图表保存路径）也会自动放行。源码 `bun run dev` 正常但**安装版只显示 “1 image” 无图**时，多为 Tauri CSP 未放行本地 server 图片 URL（已在 `tauri.conf.json` 的 `img-src` 修复，需重新打包安装）。若仍失败，确认 PNG 文件存在且路径与回复一致。
