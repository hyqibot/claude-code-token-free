...
- **Docs Pages**：私有仓 `claude-code-private` 的 Deploy VitePress Docs 仅 `docs:build` 校验；公开仓 `claude-code-token-free` 同 workflow 部署 Pages（须 Settings → Pages → GitHub Actions）。`docs/reference/` 仅 `fixes.md`、`project-structure.md` 对用户可见，策略见 `scripts/packaging/docs-publish-policy.json`。
