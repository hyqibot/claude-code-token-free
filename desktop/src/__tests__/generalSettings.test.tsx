...
describe('Settings > General tab', () => {
  ...
  it('requests native notification permission when desktop notifications are enabled', async () => {
    ...
    expect(desktopNotificationsMock.notifyDesktop).toHaveBeenCalledWith({
      title: 'Claude Code Token Free notifications are enabled',
...
describe('Settings > About tab', () => {
  beforeEach(() => {
    ...
    useUpdateStore.setState({
      ...
      releaseNotes: '# Claude Code Token Free v0.1.5\n\n- Fixed updater rendering\n- Added markdown support',
      progressPercent: 0,
      downloadedBytes: 0,
      totalBytes: null,
      error: null,
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: PRODUCT_DISPLAY_NAME, level: 1 })).toBeInTheDocument()
    expect(await screen.findByRole('heading', { name: 'Claude Code Token Free v0.1.5' })).toBeInTheDocument()
