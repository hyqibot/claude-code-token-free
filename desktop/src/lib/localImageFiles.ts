...
export function localFileImageUrl(filePath: string): string {
  return `${getBaseUrl()}/api/filesystem/file?path=${encodeURIComponent(normalizeLocalPath(filePath))}`
}
...
export function rewriteLocalImageSrc(src: string | null | undefined): string | null | undefined {
  if (!src || !isLocalImagePath(src)) return src
  return localFileImageUrl(src)
}
