...
export const adaptersApi = {
  ...
  setImRuntimeDefault(selection: { providerId: string | null; modelId: string }) {
    return api.put<{ providerId: string | null; modelId: string; source: 'draft' | 'global' }>(
      '/api/adapters/im-runtime-default',
      selection,
    )
  },

  clearImRuntimeDefault() {
    return api.put<{ providerId: string | null; modelId: string; source: 'draft' | 'global' }>(
      '/api/adapters/im-runtime-default',
      { clear: true },
    )
