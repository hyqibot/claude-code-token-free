...
describe('settingsStore insecureTls', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    window.localStorage.clear()
  ...
  it('defaults insecureTls to false and reflects user settings on fetchAll', async () => {
    vi.doMock('../api/settings', () => ({
      settingsApi: {
        getUser: vi.fn().mockResolvedValue({ insecureTls: true }),
        updateUser: vi.fn(),
        getPermissionMode: vi.fn().mockResolvedValue({ mode: 'default' }),
        setPermissionMode: vi.fn(),
        getCliLauncherStatus: vi.fn(),
  ...
  it('setInsecureTls(true) persists via updateUser and rolls back on failure', async () => {
    const updateUser = vi
      .fn()
    ...
    await useSettingsStore.getState().setInsecureTls(true)
    expect(updateUser).toHaveBeenLastCalledWith({ insecureTls: true })
    expect(useSettingsStore.getState().insecureTls).toBe(true)

    await useSettingsStore.getState().setInsecureTls(false)
    expect(updateUser).toHaveBeenLastCalledWith({ insecureTls: false })
    expect(useSettingsStore.getState().insecureTls).toBe(true)
  })
})
