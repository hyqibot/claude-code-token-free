...
import { rewriteLocalImageSrc } from '../../lib/localImageFiles'
...
function enhanceMarkdownHtml(html: string, sessionId?: string | null): string {
  ...
  container.querySelectorAll('img[src]').forEach((img) => {
    ...
    const rewritten = rewriteLocalImageSrc(src, sessionId)
...
export function MarkdownRenderer({ content, variant = 'default', className, sessionId }: Props) {
  ...
  if (codeBlocks.length === 0) {
    const cleanHtml = enhanceMarkdownHtml(html, sessionId)
    return (
      <div
  ...
  return (
    <div className={proseClasses} onClick={handleClick}>
      {parts.map((part, i) =>
        part.type === 'html' ? (
          <div key={i} dangerouslySetInnerHTML={{ __html: enhanceMarkdownHtml(part.content, sessionId) }} />
