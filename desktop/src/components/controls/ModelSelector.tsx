...
import { ZERO_TOKEN_CANONICAL_MODEL_IDS } from '../../constants/zeroTokenWebModels'
...
function buildProviderModels(
  ...
  /** 预设四槽位均为同一 id 时只会剩 1 项；Zero-Token 需列出网关全部 canonical 网页模型 */
  if (provider.presetId === 'zero-token-web') {
    return ZERO_TOKEN_CANONICAL_MODEL_IDS.map((id) => {
      const lk = ZERO_TOKEN_MODEL_LABEL_KEY[id]
      return {
        id,
        name: lk ? t(lk) : id,
        description: '',
...
function buildProviderChoices(
  ...
  const zeroTokenProviders = providers.filter((p) => p.presetId === 'zero-token-web')
  const otherProviders = providers.filter((p) => p.presetId !== 'zero-token-web')
  const mapRow = (provider: SavedProvider) => ({
    providerId: provider.id,
    providerName: provider.name,
    isDefault: activeId === provider.id,
    models: buildProviderModels(provider, labels, t),
  })
  return [
    officialChoices(availableModels, activeId === null, officialName),
    ...zeroTokenProviders.map(mapRow),
    ...otherProviders.map(mapRow),
  ]
}
...
export function ModelSelector({
  ...
  const handleRuntimeSelect = (selection: RuntimeSelection) => {
    ...
    const { activeId, providers } = useProviderStore.getState()
    const mid = selection.modelId.trim()
    if (
      mid &&
      selection.providerId !== null &&
      selection.providerId === activeId
    ) {
      const row = providers.find((p) => p.id === selection.providerId)
      if (row?.presetId === 'zero-token-web') {
        void useSettingsStore.getState().setModel(mid)
      }
    }
    setOpen(false)
  }
