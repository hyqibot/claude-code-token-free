...
export function AssistantMessage({ content, isStreaming, sessionId }: Props) {
    ...
    <div className="group mb-5 flex justify-start">
      ...
      >
        <div className={`rounded-[20px] rounded-tl-[8px] border border-[var(--color-border)]/60 bg-[var(--color-surface)] px-4 py-3 text-sm text-[var(--color-text-primary)] shadow-sm ${
          ...
          <MarkdownRenderer content={content} variant={documentLayout ? 'document' : 'default'} sessionId={sessionId} />
          {!isStreaming && <InlineImageGallery text={content} sessionId={sessionId} />}
          {isStreaming && (
