    ...
    >
      <div className={`px-3 pb-2 ${isTauri && !isWindows ? 'pt-[44px]' : 'pt-3'}`}>
        <div className={`flex ${sidebarOpen ? 'items-center justify-between gap-3' : 'flex-col items-center gap-2'}`}>
          <div className={`flex min-w-0 flex-1 items-start ${sidebarOpen ? 'gap-2.5' : 'justify-center'}`}>
            <img src="/app-icon.png" alt="" className="mt-0.5 h-8 w-8 flex-shrink-0" />
            <span
              className={`sidebar-copy sidebar-brand ${sidebarOpen ? 'sidebar-copy--visible' : 'sidebar-copy--hidden'} text-[13px] font-semibold leading-tight tracking-tight text-[var(--color-text-primary)]`}
              style={{ fontFamily: 'var(--font-headline)' }}
            >
              <span className="block">{PRODUCT_DISPLAY_NAME}</span>
              <span className="block text-[var(--color-primary-container)]">Free Token💘+钉钉iclaw001入裙</span>
            </span>
          </div>
          <div className={`flex flex-shrink-0 items-center ${sidebarOpen ? 'gap-1.5' : 'flex-col gap-2'}`}>
            <a
              href="https://www.dingtalk.com/download?action=joingroup&code=v1,k1,9O3Nk5uBqF+FKGHas0gK4dkuLhC1CkMJ4CgU45rKMf8=&_dt_no_comment=1&origin=11"
              target="_blank"
