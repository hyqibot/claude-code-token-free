...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Claude Code Token Free")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Claude Code Token Free")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Claude Code Token Free")
        .menu(&menu)
...
/// Sidecar 进程（server + IM adapters）必须读写同一份 ~/.claude/adapters.json。
/// Windows 打包环境下 Bun sidecar 的 os.homedir() 可能与桌面主进程不一致，
/// 因此显式注入 CLAUDE_CONFIG_DIR / USERPROFILE / HOME。
fn inject_claude_config_env(env: &mut HashMap<String, String>) {
    if let Some(home) = home_dir() {
        let home_str = home.to_string_lossy().to_string();
        env.entry("USERPROFILE".to_string())
            .or_insert_with(|| home_str.clone());
        env.entry("HOME".to_string())
            .or_insert_with(|| home_str);
    }
...
fn sidecar_environment() -> HashMap<String, String> {
    let mut env = terminal_environment(&default_shell());
    inject_claude_config_env(&mut env);
    env
}
...
fn start_server_sidecar(app: &AppHandle) -> Result<ServerRuntime, String> {
    ...
    for (key, value) in sidecar_environment() {
...
fn kill_windows_sidecars() {
    for image_name in [
        ...
        "claude-sidecar.exe",
        "zero-token-gateway-x86_64-pc-windows-msvc.exe",
        "zero-token-gateway-aarch64-pc-windows-msvc.exe",
        "zero-token-gateway.exe",
        "zero-token-webauth-runner-x86_64-pc-windows-msvc.exe",
        "zero-token-webauth-runner-aarch64-pc-windows-msvc.exe",
        "zero-token-webauth-runner.exe",
    ] {
...
mod tests {
    use super::{
        ...
        has_meaningful_intersection, inject_claude_config_env, is_persistable_window_state,
        parse_env_block, run_notification_bridge, StoredWindowState,
    };
    ...
    #[test]
    fn sidecar_environment_injects_claude_config_dir() {
        let mut env = HashMap::new();
        inject_claude_config_env(&mut env);

        let config_dir = env
            .get("CLAUDE_CONFIG_DIR")
            .expect("CLAUDE_CONFIG_DIR should be set for sidecars");
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Claude Code Token Free").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Claude Code Token Free")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
