...
APP_BUNDLE_NAME="Claude Code token free.app"
APP_BUNDLE_ID="com.claude-code-haha.desktop"

usage() {
  cat <<'EOF'
Build Claude Code token free desktop for macOS Apple Silicon and output a DMG.
...
build_canonical_dmg() {
  ...
  hdiutil create \
    -volname "Claude Code token free" \
...
tell application "Finder"
  tell disk "Claude Code token free"
    open
...
if [[ -n "${LATEST_APP}" ]]; then
  ...
  # Without this, opening target/.../bundle/macos/Claude Code token free.app directly
  ...
  CANONICAL_DMG="${CANONICAL_OUTPUT_DIR}/$(basename "${LATEST_DMG:-Claude Code token free_0.1.0_aarch64.dmg}")"
  build_canonical_dmg \
