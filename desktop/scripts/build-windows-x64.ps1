...
function Get-StagedArtifactName {
  ...
  $prefix = "Claude Code Free Token v$appVersion Windows x64"

  switch -Regex ($ArtifactName) {
    '^latest\.json$' { return 'latest.json' }
    '\.exe\.sig$' { return "$prefix-setup.exe.sig" }
    '\.exe$' { return "$prefix-setup.exe" }
    '\.nsis\.zip\.sig$' { return "$prefix.nsis.zip.sig" }
    '\.nsis\.zip$' { return "$prefix.nsis.zip" }
    '\.msi\.zip\.sig$' { return "$prefix.msi.zip.sig" }
    '\.msi\.zip$' { return "$prefix.msi.zip" }
    '\.msi\.sig$' { return "$prefix.msi.sig" }
    '\.msi$' { return "$prefix.msi" }
