...
import { applyImRuntimeDefault, syncImRuntimeOnBridgeReady } from '../common/im-runtime.js'
...
async function ensureSession(chatId: string): Promise<boolean> {
  ...
  if (stored) {
    ...
    if (!ready) await sendText(chatId, 'Connection timed out. Send /new to reconnect.')
    if (ready) await syncImRuntimeOnBridgeReady(bridge, httpClient, chatId)
    return ready
  }
