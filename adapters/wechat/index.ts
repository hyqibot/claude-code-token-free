...
async function dispatchOutboundImage(chatId: string, pending: PendingUpload): Promise<void> {
  ...
  for (let attempt = 0; attempt < OUTBOUND_IMAGE_MAX_ATTEMPTS; attempt++) {
    ...
    if (pending.source.kind === 'path' && attempt === 0) {
      ...
      console.log(`[WeChat] Outbound image queued: ${label}`)
    }
    ...
    try {
      ...
      const contextToken = contextTokens.get(chatId)
      try {
        await media.uploadOutboundImage({
          baseUrl,
          token: botToken,
          toUserId: chatId,
          contextToken,
        ...
        if (!contextToken) throw err
        console.warn('[WeChat] sendImage with context token failed, retrying without context:', err instanceof Error ? err.message : err)
        await media.uploadOutboundImage({
          baseUrl,
          token: botToken,
          toUserId: chatId,
      ...
      console.log(`[WeChat] Sent outbound image to ${redactChatId(chatId)}`)
      return
    } catch (err) {
      uploaded.delete(dedupeId)
...
async function ensureSession(chatId: string): Promise<boolean> {
  ...
  if (stored) {
    ...
    if (!ready) await sendText(chatId, '连接服务器超时，请发送 /new 重新连接。')
...
async function routeUserMessage(message: WechatMessage): Promise<void> {
  ...
  enqueueWechat(chatId, async () => {
    ...
    if (!ready) return
    if (!hasAttachments && userRequestsImageResend(text)) {
