...
export class WechatMediaService {
  ...
  async uploadOutboundImage(params: {
    baseUrl: string
    token: string
    toUserId: string
    contextToken?: string
    ...
    const encrypted = encryptWechatAesEcb(params.buffer, aesKey)
    const upload = await getWechatUploadUrl({
      baseUrl: params.baseUrl,
      token: params.token,
      toUserId: params.toUserId,
      rawBuffer: params.buffer,
      aesKey,
      contextToken: params.contextToken,
    })
    const encryptQueryParam = await uploadWechatCdn({
