...
export function computeWechatEncryptedSize(rawSize: number): number {
  return Math.ceil((rawSize + 1) / 16) * 16
}

export function buildWechatAesKeyFields(aesKey: Buffer): { aeskeyHex: string; aesKeyField: string } {
  const aeskeyHex = aesKey.toString('hex')
  return {
    aeskeyHex,
    aesKeyField: Buffer.from(aeskeyHex, 'utf-8').toString('base64'),
  }
}

export function encryptWechatAesEcb(plaintext: Buffer, key: Buffer): Buffer {
...
export async function getWechatUploadUrl(params: {
  ...
  const { aesKeyField } = buildWechatAesKeyFields(params.aesKey)
  const filekey = crypto.randomBytes(16).toString('hex')
  const rawsize = params.rawBuffer.length
  const encryptedSize = computeWechatEncryptedSize(rawsize)
  const rawfilemd5 = crypto.createHash('md5').update(params.rawBuffer).digest('hex')
  const aeskeyHex = params.aesKey.toString('hex')
  ...
  const rawText = await apiPostFetch({
    ...
    token: params.token,
    timeoutMs: params.timeoutMs ?? API_TIMEOUT_MS,
    label: 'wechatGetUploadUrl',
  })
  assertWechatApiOk(rawText, 'wechatGetUploadUrl')
  const body = JSON.parse(rawText) as Record<string, unknown>
  const uploadUrl = resolveWechatCdnUploadUrl(body, filekey)
  if (!uploadUrl) {
    throw new Error(`wechatGetUploadUrl missing upload target: ${rawText.slice(0, 300)}`)
  }
...
function resolveWechatCdnUploadUrl(body: Record<string, unknown>, filekey: string): string | null {
  const fullUrl = typeof body.upload_full_url === 'string'
    ? body.upload_full_url
    : typeof body.uploadFullUrl === 'string'
      ? body.uploadFullUrl
      : undefined
  if (fullUrl) return fullUrl

  const uploadParam = typeof body.upload_param === 'string'
    ? body.upload_param
    : typeof body.uploadParam === 'string'
      ? body.uploadParam
      : undefined
  if (!uploadParam) return null

  const url = new URL(`${WECHAT_CDN_BASE_URL}/upload`)
  url.searchParams.set('encrypted_query_param', uploadParam)
  url.searchParams.set('filekey', filekey)
  return url.toString()
}

export async function uploadWechatCdn(params: {
  uploadUrl: string
  encrypted: Buffer
  timeoutMs?: number
}): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`wechatCdnUpload ${res.status}: ${text}`)
    }
    const encryptQueryParam = res.headers.get('x-encrypted-param')
    if (!encryptQueryParam) {
      throw new Error('wechatCdnUpload missing x-encrypted-param response header')
    }
...
export async function sendWechatImage(params: {
  baseUrl: string
  token: string
  to: string
  contextToken?: string
  ...
  const rawText = await apiPostFetch({
    ...
    token: params.token,
    timeoutMs: params.timeoutMs ?? API_TIMEOUT_MS,
    label: 'wechatSendImage',
  })
  assertWechatApiOk(rawText, 'wechatSendImage')
}
