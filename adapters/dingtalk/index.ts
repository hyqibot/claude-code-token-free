...
import { applyImRuntimeDefault, clearImRuntimeSync, syncImRuntimeOnBridgeReady } from '../common/im-runtime.js'
import { ensureBridgeSessionOpen } from '../common/bridge-session.js'
...
import { formatOutboundImageLogLabel, resolvePendingDedupeId, resolvePendingUploadBuffer } from '../common/attachment/resolve-pending-upload.js'
import {
...
} from '../common/attachment/outbound-image-helper.js'
import {
...
const textOnlyReplyChats = new Set<string>()
const imageWatchers = new Map<string, ImageBlockWatcher>()
const uploadedImageIds = new Map<string, Set<string>>()
const lastOutboundImageMtime = new Map<string, number>()
const outboundImageTracker = new OutboundImageTracker()

const OUTBOUND_IMAGE_RETRY_MS = 400
const OUTBOUND_IMAGE_MAX_ATTEMPTS = 8

const AI_CARD_CREATE_TIMEOUT_MS = 8_000
...
function getOrCreateAiCard(chatId: string): Promise<DingTalkAiCardInstance | null> | null {
  if (textOnlyReplyChats.has(chatId)) return null
  ...
  if (!card) {
    card = Promise.race([
      aiCards.createForTarget(target),
      new Promise<null>((resolve) => setTimeout(() => resolve(null), AI_CARD_CREATE_TIMEOUT_MS)),
    ])
...
async function flushToAiCard(chatId: string, newText: string, isComplete: boolean): Promise<void> {
  ...
  if (textOnlyReplyChats.has(chatId)) {
...
function getUploadedIds(chatId: string): Set<string> {
  let set = uploadedImageIds.get(chatId)
  if (!set) {
    set = new Set()
    uploadedImageIds.set(chatId, set)
  }
  return set
}

async function dispatchOutboundImage(chatId: string, pending: PendingUpload): Promise<void> {
  const uploaded = getUploadedIds(chatId)
  const minMtimeMs = lastOutboundImageMtime.get(chatId) ?? 0

  for (let attempt = 0; attempt < OUTBOUND_IMAGE_MAX_ATTEMPTS; attempt++) {
    const resolveOpts = { minMtimeMs }
    const dedupeId = await resolvePendingDedupeId(pending, resolveOpts)
    if (uploaded.has(dedupeId)) {
      if (attempt + 1 >= OUTBOUND_IMAGE_MAX_ATTEMPTS) return
      await sleep(OUTBOUND_IMAGE_RETRY_MS)
      continue
    }
    uploaded.add(dedupeId)

    if (pending.source.kind === 'path' && attempt === 0) {
      const label = await formatOutboundImageLogLabel(pending, resolveOpts)
      console.log(`[DingTalk] Outbound image queued: ${label}`)
    }

    const sessionWebhook = sessionWebhooks.get(chatId)
    if (!sessionWebhook) {
      console.warn(`[DingTalk] Missing sessionWebhook for ${chatId}; cannot send outbound image`)
      uploaded.delete(dedupeId)
      return
    }

    try {
      const { buffer, mime, resolvedPath } = await resolvePendingUploadBuffer(pending, resolveOpts)
      const check = checkAttachmentLimit('image', buffer.length, mime)
      if (!check.ok) {
        console.warn('[DingTalk] Outbound image rejected:', check.hint)
        uploaded.delete(dedupeId)
        return
      }

      const token = await getAccessToken()
      const ext = mime.includes('jpeg') ? '.jpg' : mime.includes('gif') ? '.gif' : mime.includes('webp') ? '.webp' : '.png'
      let fileName = `outbound-${Date.now()}${ext}`
      if (pending.source.kind === 'path') {
        fileName = path.basename(resolvedPath ?? pending.source.path)
      }
      const mediaId = await media.uploadImage(buffer, fileName, token)
      await media.sendImageMessage(sessionWebhook, token, mediaId, fileName)
      if (resolvedPath) {
        const stat = await fs.stat(resolvedPath)
        lastOutboundImageMtime.set(chatId, stat.mtimeMs)
      }
      console.log(`[DingTalk] Sent outbound image to ${chatId}`)
      return
    } catch (err) {
      uploaded.delete(dedupeId)
      if (attempt + 1 >= OUTBOUND_IMAGE_MAX_ATTEMPTS) {
        console.error('[DingTalk] dispatchOutboundImage failed:', err instanceof Error ? err.message : err)
        return
      }
      await sleep(OUTBOUND_IMAGE_RETRY_MS)
    }
...
function queueOutboundImageText(chatId: string, text: string, opts?: { complete?: boolean }): void {
  feedOutboundImageText(chatId, text, getImageWatcher(chatId), outboundImageTracker, (id, pending) => {
    void dispatchOutboundImage(id, pending)
  }, opts)
}

function flushOutboundImageQueue(chatId: string): void {
  flushOutboundImages(chatId, getImageWatcher(chatId), outboundImageTracker, (id, pending) => {
    void dispatchOutboundImage(id, pending)
  })
}

function resendCachedImages(chatId: string): void {
  for (const pending of outboundImageTracker.pendingFromSession(chatId)) {
    getUploadedIds(chatId).delete(pending.id)
...
function clearTransientChatState(chatId: string): void {
  ...
  textOnlyReplyChats.delete(chatId)
  imageWatchers.delete(chatId)
  uploadedImageIds.delete(chatId)
  lastOutboundImageMtime.delete(chatId)
  outboundImageTracker.clear(chatId)
  clearImRuntimeSync(chatId)
...
async function ensureExistingSession(chatId: string): Promise<{ sessionId: string; workDir: string } | null> {
  ...
  const ready = await ensureBridgeSessionOpen(
    bridge,
    chatId,
    stored,
    (msg) => handleServerMessage(chatId, msg),
  )
...
async function ensureSession(chatId: string): Promise<boolean> {
  ...
  if (stored) {
    ...
    if (!ready) await sendText(chatId, '⚠️ 连接服务器超时，请发送 /new 重新连接。')
    if (ready) await syncImRuntimeOnBridgeReady(bridge, httpClient, chatId, stored.sessionId)
    return ready
  }
  ...
  switch (msg.type) {
    ...
    case 'content_start':
      if (msg.blockType === 'text') {
        if (runtime.state !== 'idle') {
          flushOutboundImageQueue(chatId)
        }
        getImageWatcher(chatId).reset()
    ...
    case 'content_delta':
      if (typeof msg.text === 'string' && msg.text) {
        getAiCardBuffer(chatId).append(msg.text)
        queueOutboundImageText(chatId, msg.text)
      }
      break
    case 'tool_use_complete':
      ...
      break
    case 'tool_result':
      queueOutboundImageText(chatId, toolResultContentToText(msg.content), { complete: true })
      break
...
async function routeUserMessage(chatId: string, text: string, attachments: AttachmentRef[] = []): Promise<void> {
  enqueue(chatId, async () => {
    try {
      await routeUserMessageInner(chatId, text, attachments)
    } catch (err) {
      console.error(`[DingTalk] Failed to handle message for ${chatId}:`, err)
      try {
        await sendText(chatId, `处理消息失败：${err instanceof Error ? err.message : String(err)}`)
      } catch (sendErr) {
        console.error(`[DingTalk] Failed to report message handling error for ${chatId}:`, sendErr)
      }
    }
  })
}

async function routeUserMessageInner(chatId: string, text: string, attachments: AttachmentRef[] = []): Promise<void> {
    ...
    if (!ready) return
    if (!hasAttachments && userRequestsImageResend(trimmed)) {
      resendCachedImages(chatId)
    }
    const effectiveText = trimmed || (attachments.length > 0 ? '(用户发送了附件)' : '')
    if (!effectiveText && attachments.length === 0) return
    getUploadedIds(chatId).clear()
...
async function handleRobotMessage(data: DingTalkRobotMessage): Promise<void> {
  if (!isDingTalkDirectMessage(data)) {
    console.log(`[DingTalk] Ignored non-DM message (conversationType=${String(data.conversationType)})`)
    return
  }
  ...
  if (!chatId || !userId || (!text && mediaCandidates.length === 0)) {
    console.warn('[DingTalk] Ignored robot message: missing chatId/userId/text')
    return
  }

  console.log(`[DingTalk] Received from ${userId}: ${text.slice(0, 80)}`)
...
async function start(): Promise<void> {
  ...
  client.registerCallbackListener(TOPIC_ROBOT, async (res: any) => {
    ...
    if (!data) {
      console.warn('[DingTalk] Ignored robot payload: unparseable data')
      return
    }
