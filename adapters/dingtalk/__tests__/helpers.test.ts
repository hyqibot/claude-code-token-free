...
describe('DingTalk helpers', () => {
  ...
  it('accepts numeric conversationType for direct messages', () => {
    expect(isDingTalkDirectMessage({ conversationType: 1, senderId: 'u-1' })).toBe(true)
  })

  it('unwraps nested stream payload shapes', () => {
    const nested = {
      data: {
        conversationType: '1',
    ...
    const parsed = parseDingTalkPayload(nested)
    expect(parsed?.senderStaffId).toBe('staff-2')
    expect(extractDingTalkText(parsed!)).toBe('hello')
  })
