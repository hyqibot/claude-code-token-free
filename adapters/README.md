...
Agent 流式文本里的 markdown 图片引用 `![alt](path|url|data:)` 以及正文中的绝对路径（如 `D:/CCwork/chart.png`）会被 `ImageBlockWatcher` 识别、上传到 IM 平台,作为独立图片消息发出:

- 飞书: `im.message.create(msg_type='image')` 单发(card 内嵌是后续优化)
- Telegram: `bot.api.sendPhoto(InputFile)` 单发
- 微信: iLink `getuploadurl` → CDN AES 加密上传 → `sendmessage` 图片消息
- 钉钉: `oapi.dingtalk.com/media/upload` → sessionWebhook `msgtype=image`
