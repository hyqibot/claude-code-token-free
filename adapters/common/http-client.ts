...
export class AdapterHttpClient {
  ...
  async createSession(workDir: string): Promise<string> {
    ...
    try {
      ...
      if (!data.sessionId) {
        throw new Error('Failed to create session: missing sessionId')
      }
  ...
  async getImRuntimeDefault(): Promise<{
    ...
    const { controller, timer } = this.createTimeoutController()
    try {
      const res = await fetch(`${this.httpBaseUrl}/api/adapters/im-runtime-default`, {
        signal: controller.signal,
      })
      if (!res.ok) {
        throw new Error(`Failed to load IM runtime default: ${res.statusText}`)
      }
      const text = await res.text()
      ...
      if (typeof data.modelId !== 'string' || !data.modelId.trim()) return null
      return {
