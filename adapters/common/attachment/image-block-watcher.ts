...
import { extractLocalImagePaths, normalizeLocalPath } from './local-image-paths.js'
...
function classify(target: string): PendingUpload['source'] | null {
  ...
  if (/^[A-Za-z]:[\\/]/.test(target) || target.startsWith('~/') || target.startsWith('~\\')) {
    return { kind: 'path', path: normalizeLocalPath(target) }
  }
...
export class ImageBlockWatcher {
  ...
  feed(chunk: string): PendingUpload[] {
    ...
    // 本地盘符路径只在 reconcile() 扫描完整 streamText，避免 UTF-8 分片产生乱码路径。
    return out
  }

  /** Scan full streamed text at end-of-turn for paths missed during chunking. */
  reconcile(): PendingUpload[] {
    return this.emitPaths(extractLocalImagePaths(this.streamText))
  }
  ...
  private emitPaths(paths: string[]): PendingUpload[] {
    const out: PendingUpload[] = []
    for (const path of paths) {
      const id = fingerprint(`path:${path}`)
      if (this.seen.has(id)) continue
      this.seen.add(id)
      const pending: PendingUpload = { id, source: { kind: 'path', path } }
      out.push(pending)
      this.accumulated.push(pending)
    }
    return out
