...
describe('AdapterHttpClient', () => {
  ...
  it('createSession rejects empty success bodies', async () => {
    globalThis.fetch = mock(() =>
      Promise.resolve(new Response('', { status: 201 })),
    ) as any

    await expect(client.createSession('/path/to/project')).rejects.toThrow('empty response body')
  })

  it('getImRuntimeDefault calls GET /api/adapters/im-runtime-default', async () => {
    globalThis.fetch = mock(() =>
    ...
    const runtime = await client.getImRuntimeDefault()
