...
describe('WsBridge', () => {
  ...
  it('sendRuntimeConfig returns false when no open connection', () => {
    ...
    expect(bridge.sendRuntimeConfig('chat-runtime', 'provider-1', 'kimi-k2.6')).toBe(false)
  })

  it('isSessionOpen is false while connecting and true after open', async () => {
    const server = new WebSocketServer({ port: 0 })
    await new Promise<void>((resolve) => server.on('listening', () => resolve()))
    const port = (server.address() as { port: number }).port
    server.on('connection', () => {})

    const liveBridge = new WsBridge(`ws://127.0.0.1:${port}`, 'test')
    liveBridge.connectSession('chat-open', 'sess-open')
    expect(liveBridge.isSessionOpen('chat-open')).toBe(false)

    const opened = await liveBridge.waitForOpen('chat-open')
    expect(opened).toBe(true)
    expect(liveBridge.isSessionOpen('chat-open')).toBe(true)
